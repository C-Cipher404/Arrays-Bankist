...
btnClose.addEventListener('click', function (e) {
  ...
  if (
    ...
    Number(inputClose.value) === currentAccount.pin
  ) {
    const index = accounts.findIndex(
      acc => acc.useranme === currentAccount.username
    );
    // DELETE ACCOUNT
...
containerMovements.forEach(function (value, _, map) {
...
const humanage = '';
const dogAge = humanage * 2;

const dog = dogAge.filter(dog => dog > 18);
...
const { deposits, withdrawls } = accounts
  ...
  .reduce(
    (sums, cur) => {
      ...
      //the above is cleaner and better than bottom
...
const convertTitleCase = function (title) {
  ...
  const titleCase = title
    ...
    .split(' ')
    .map(word =>
      expections.includes(word) ? word : captizalize(word)
    )
    .join(' ');
...
// ("Matlida and Alice and Bob's dogs eat too much!");
